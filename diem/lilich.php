			...
			if (isset($_POST['themhs'])) {
				...
				if (strlen(trim($_POST['hoten']))<4) { //ten qua ngan
					...
					if (dbcount2("qlt_dshocsinh","hoten='".$_POST['hoten']."' and ngaysinh='".$_POST[ngaysinh]."'")==0) {
						...
						$sql="insert into qlt_dshocsinh (stt,hoten,ngaysinh,gioinu,lop,noisinh,he,dcthon,dcxa,sodt,hotencha,nghecha,hotenme,ngheme,doanvien,conlietsi,conthuongbinh,conchinhsach,ghichu) value (";
						$sql.=" ".$stt.",'".$_POST['hoten']."','".$_POST['nam']."-".$_POST['thang']."-".$_POST['ngay']."',".($_POST['nu']?1:0).",'".$lopcn."','".$_POST['noisinh']."','".$_POST['he']."','".$thon."','".$xa."','";
						$sql.=$_POST['sodt']."','".$_POST['hotencha']."','".$_POST['nghecha']."','".$_POST['hotenme']."','".$_POST['ngheme']."',".($_POST['doanvien']?1:0).",".($_POST['conlietsi']?1:0).",'".$_POST['conthuongbinh']."','".$_POST['conchinhsach']."','".$_POST['ghichu']."')";
						//echo $sql; 
						$result=dbquery($sql);
				...
				echo  <<<cb
<select name="dsxa" id="dsxaid" onchange="DropDownTextToBox(this,'dcxaid');">
cb;
...
cb;
		...
		} elseif (($_GET['act']=="del")&&isset($_GET['stt'])) { //Xoá học sinh
			$result=dbquery("delete from qlt_dshocsinh where lop='".$lopcn."' and stt=".$_GET['stt']);
			$result=dbquery("update qlt_dshocsinh set stt=stt-1 where stt>".$_GET['stt']);
			redirect(FUSION_SELF."?lop=".$lopcn);
		} elseif (($_GET['act']=="edit")&&isset($_GET['stt'])) {//sửa học sinh
			//c
			if (isset($_POST['luu'])) {
				//echo "Cập nhật dữ liệu";
				if (strlen(trim($_POST['hoten']))<4) { //ten qua ngan
				...
				$result=dbquery("select * from qlt_dshocsinh where lop='".$lopcn."' and stt=".$_GET['stt']." limit 1");
				if (dbrows($result)) {
					$data=dbarray($result);
...
cb;
			...
			$stt=$_GET['stt'];
			$result=dbquery("update qlt_dshocsinh set stt=-stt+2*".$stt."+1 where lop='".$lopcn."' and (stt=".$stt." or stt=".($stt+1).")");
			redirect(FUSION_SELF."?lop=".$lopcn);
		...
		$result=dbquery("select * from qlt_dshocsinh where lop='".$lopcn."' order by stt");
		opentable("Danh sách học sinh lớp ".$lopcn);
		if (dbrows($result)) {//liet ke ds hs lopcn
			echo "<table class='tbl-border small center'><tr class='tbl3' align=center  class='toosmall'><td class='toosmall'>Mã HS</td><td>TT</td><td  class='toosmall'>Họ và tên</td><td class='toosmall'>Hệ</td>";
			...
			while ($data=dbarray($result)) {
				$i=4-$i;
				echo "<tr class='info0 tbl".$i."'><td class='toosmall'><font color=white>".$data['id']."</font></td><td class='toosmall'  align=center>".$data['stt']."</td><td class='toosmall'>".$data['hoten']."</td><td class='toosmall'>".$data['he']."</td><td class='toosmall'>".($data['gioinu']?"X":"")."</td>";
				...
				echo "<td class='toosmall'>[<a href='".FUSION_SELF."?lop=".$data['lop']."&act=del&stt=".$data['stt']."'>Xoá</a>] [<a href='".FUSION_SELF."?lop=".$data['lop']."&act=edit&stt=".$data['stt']."'>Sửa</a>]";
				echo "<a href='".FUSION_SELF."?lop=".$data['lop']."&act=movedown&stt=".$data['stt']."'><img alt='[xuống]' title='xuống' src='".get_image("down")."'></a><a href='".FUSION_SELF."?lop=".$data['lop']."&act=moveup&stt=".$data['stt']."'><img alt='[lên]' title='lên' src='".get_image("up")."'></a></td></tr>";
			}
			echo "<tr><td>";
			echo "</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td>[<a href='".FUSION_SELF."?lop=".$lopcn."&act=add'>Thêm học sinh</a>]";
			echo "</td></tr>";
			echo "</table>";
		} else {
	...
	//echo "ABC";
	if (iMOD && checkrights("DS")) {
		echo "Chọn lớp:<br>";
		$result=dbquery("select qlt_dslop.*,qlt_users.user_name from qlt_dslop,qlt_users where qlt_dslop.gvcn=qlt_users.user_id order by stt");
		if (dbrows($result)) {
