...
if (!isset($_GET['group_id']) || !isnum($_GET['group_id'])) {
        ...
        } else if(stristr($user_data['user_avatar'], "[".$user_data['user_id']."].")) {
        ...
        if (file_exists(IMAGES.$user_avatar)) {
                echo "<td rowspan='5' width='1%' class='tbl'><img src='".IMAGES.$user_avatar."' alt='' /></td>\n";
        }
        echo "<td width='1%' class='tbl1' style='white-space:nowrap'>".$locale['u001']."</td>\n";
        echo "<td align='right' class='tbl1'><strong>".color_group($user_data['user_name'],$user_data['user_level'])." <img src='images/".$user_data['user_level'].".gif' border=0></td>\n";
        echo "</tr>\n<tr>\n";
        ...
        echo "</tr>\n<tr>\n";
        echo "<td colspan='3' class='tbl2' style='text-align:center;white-space:nowrap'><a href='messages.php?msg_send=".$user_data['user_id']."' title='".$locale['u043']."'>".$locale['u043']."</a></td>\n";
        echo "</tr>\n</table>\n";
        ...
        if (iADMIN && checkrights("M") && $user_data['user_id'] != $userdata['user_id']) {
                $user_groups_opts = "";
                if ($user_data['user_level'] < 102 or iSUPERADMIN) {
                        echo "<div style='margin:5px'></div>\n";
                        ...
                        if (dbrows($result)) {
                                ...
                                if (iADMIN && checkrights("UG") && $user_groups_opts) {
                                        ...
                                        echo "<select name='user_group' class='textbox'>\n".$user_groups_opts."</select>\n";
        ...
        echo "<br /><table cellpadding='0' cellspacing='1' width='100%' class='tbl-border center'>\n<tr>\n";
        echo "<td><span style='margin:5px;'>::
                <a href='".BASEDIR."my_threads.php?user_id=".$user_data['user_id']."&action=myposts'><span style='font-size: 11px'>".$locale['425']."<strong>".$user_data['user_name']."</strong></span></a> ::</span>
                <br /><br /><span style='margin:5px; font-size: 11px'>::
                <a href='".BASEDIR."my_threads.php?user_id=".$user_data['user_id']."&action=mythreads'><span style='font-size: 11px'>".$locale['426']."<strong>".$user_data['user_name']."</strong></span></a> ::</span>\n";
        echo "</td></tr>";
        echo "</table></center>";
} else {
