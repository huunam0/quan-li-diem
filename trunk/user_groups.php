...
if ((!checkrights("UG") )&&($userdata['user_level']<102)) { redirect("index.php"); }
...
if (isset($_POST['save_group'])) {
        ...
        if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
                ...
                $result = dbquery("INSERT INTO ".DB_USER_GROUPS." (group_name, group_description,donvi) VALUES ('$group_name', '$group_description',$madonvi)");
        ...
        if (dbcount("(*)", "users", "user_groups REGEXP('^\\\.{$_GET['group_id']}$|\\\.{$_GET['group_id']}\\\.|\\\.{$_GET['group_id']}$')") != 0) {
        ...
        $result = dbquery("SELECT * FROM ".DB_USER_GROUPS." where donvi=$madonvi ORDER BY group_name");
        if (dbrows($result) != 0) {
        ...
        if (isset($_GET['group_id']) && isnum($_GET['group_id'])) {
                opentable($locale['440']);
                        ...
                        $result = dbquery("SELECT user_id,user_name,user_groups,user_level FROM qlt_users WHERE   donvi=$madonvi ORDER BY user_level DESC, user_name");
                        if (isset($result) && dbrows($result)) {
                                ...
                                while ($data = dbarray($result)) {
                                        if (!preg_match("(^\.{$_GET['group_id']}$|\.{$_GET['group_id']}\.|\.{$_GET['group_id']}$)", $data['user_groups'])) {
                                                ...
                                                $users .= "<tr>\n<td class='$row_color'><input type='checkbox' name='add_check_mark[]' value='".$data['user_id']."' /> ".$data['user_name']."</td>\n<td align='right' width='1%' class='$row_color' style='white-space:nowrap'>".getuserlevel($data['user_level'])."</td>\n</tr>";
                                        }
                                }
                                if ($i > 0) {
                ...
                if ($rows) {
                        ...
                        while ($data = dbarray($result)) {
                                $row_color = ($i % 2 == 0 ? "tbl1" : "tbl2"); $i++;
                                echo "<tr>\n<td class='$row_color'><input type='checkbox' name='rem_check_mark[]' value='".$data['user_id']."' /> ".$data['user_name']."</td>\n<td align='right' width='1%' class='$row_color' style='white-space:nowrap'>".getuserlevel($data['user_level'])."</td>\n</tr>";
                        }
                        echo "<tr>\n<td colspan='2' class='tbl1'>\n";
