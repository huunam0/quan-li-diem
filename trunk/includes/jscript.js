...
function addFavorite2(){
    ...
    if(document.all) {
        window.external.AddFavorite(url, 'Forum Thptnguyendu - ' + title);
    } else if(window.sidebar) {
        window.sidebar.addPanel('Forum Thptnguyendu - ' + title, url, '');
    }
}
...
function opacityit(obj){
    if(start > end) {
        ...
        if (currentOpac<end){
            currentOpac = currentOpac + step;
            changeOpac(obj,currentOpac);
        } else if (window.changing) clearInterval(changing);
    }
}
function logout(){
    if(confirm('Bạn thật sự muốn đăng xuất khỏi hệ thống !?')) return true;
    else return false;
}
