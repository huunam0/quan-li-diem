...
if (!function_exists("generate_search_opts")) {
	function generate_search_opts($textarea_name, $inputform_name) {
		global $settings;
		include_once LOCALE.LOCALESET."search.php";
    $generated = "<input type='button' value='".$locale['407']."' class='button' style='width:100px' onclick=\"addText('".$textarea_name."', '[search=all]', '[/search]', '".$inputform_name."');return false;\" /><br />";
		if ($handle = opendir(BASEDIR."includes/search")) {
			while (false !== ($file = readdir($handle))) {
				if ($file!="." && $file!=".." && $file!="index.php" && eregi("_include.php", $file)) {
...
'onmouseover'		=>	"",
'onmouseout'		=>	"",
'html_start'		=>	"<div id='bbcode_search_".$textarea_name."' class='tbl1' style='display: none; border:1px solid black; position: absolute; width: auto; height: auto; text-align: center' onclick=\"overlayclose('bbcode_search_".$textarea_name."');\">",
'includejscript'	=>	"",
'calljscript'		=>	"",
