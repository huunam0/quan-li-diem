...
if ($_GET['stype'] == "forums" || $_GET['stype']=="all") {
        ...
        $rows = dbrows($result);
        if ($rows) {
                $items_count .= THEME_BULLET."&nbsp;<a href='".FUSION_SELF."?stype=forums&amp;stext=".$_GET['stext']."&amp;".$composevars."'>".($rows == 1 ? $locale['f402'] : $locale['f403'])." ".$rows." ".$locale['522']."</a><br  />\n";
                $result = dbquery(
                        "SELECT tp.forum_id, tp.thread_id, tp.post_id, tp.post_message, tp.post_datestamp, tt.thread_subject,
                        tt.thread_sticky, tf.forum_access, tu.user_id, tu.user_name FROM ".DB_POSTS." tp
                ...
                $items_count .= THEME_BULLET."&nbsp;".$locale['f403']." 0 ".$locale['522']."<br  />\n";
        }
