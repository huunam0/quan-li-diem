<?
$bnblink = mysql_connect('localhost', 'root', '') ;
mysql_select_db('onetable');
?>
<html>
<head>
<title>combobox dependent</title>
...
$query1=dbquery("SELECT * FROM onetable where pid=0");

if($cnt=dbrows($query1))
{ 
while($list1=dbarray($query1))
{
	echo "<option value='".$list1['id']."' >".$list1['name']."</option>";
}
echo "<option value='end' >Ket thuc</option>";
}
?>
                    </select></td>
                  </tr>
