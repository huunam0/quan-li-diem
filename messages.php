                        ...
                        if (dbrows($result)) {
                                while ($data = dbarray($result)) {
                                        if ($data['user_id'] != $userdata['user_id']) {
                                                ...
                                                $message_content = str_replace("[SUBJECT]", "\"<em>".$subject."</em>\"", $locale['626']);
                                                $message_content = str_replace("[USER]", "<a href='".$settings['siteurl']."profile.php?lookup=".$userdata['user_id']."'>".$userdata['user_name']."</a>", $message_content);
                                                $send_email = isset($data['pm_email_notify']) ? $data['pm_email_notify'] : $msg_settings['pm_email_notify'];
                                                if ($send_email == "1") { sendemail($data['user_name'], $data['user_email'], $settings['siteusername'], $settings['siteemail'], $locale['625'], $data['user_name'].$message_content, "html"); }
                                        }
                                }
                        } else {
...
if (!isset($_GET['msg_send']) && !isset($_GET['msg_read']) && $_GET['folder'] != "options") {
        ...
        echo "<td class='tbl1'><input type='text' name='subject' value='".$subject."' maxlength='32' class='textbox' style='width:250px;' /></td>\n</tr>\n";
        if ($reply_message) {
        ...
        echo "<td class='tbl1'><textarea name='message' cols='75' rows='15' class='textbox' style='width:98%'>".$message."</textarea></td>\n</tr>\n";
